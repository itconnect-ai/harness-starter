...
이 셋업은 BMAD 기획·설계·구현 계획이 **먼저 완료된 뒤** 실행됩니다.
아래 3종 산출물이 이미 손에 있어야 합니다 (경로는 상관없음):
...
1. 빈 프로젝트 디렉토리 만들기: `mkdir my-project && cd my-project`
2. BMAD 산출물 3종을 `_bmad-output/planning-artifacts/` 아래에 **복사**
...
BMAD 산출물 확인 → scaffold → git → harness install → git hooks →
BMAD 스킬 설치까지 한 번에 처리됩니다.

````markdown
# Harness Greenfield — Setup 1: Bootstrap

이 세션은 **BMAD 산출물이 `_bmad-output/planning-artifacts/` 아래에
복사된 빈 프로젝트 디렉토리**에서 시작합니다. 각 단계 실패 시 STOP
...
허용 상태:
- `.git/` 없음 + `_bmad-output/` 외 비숨김 파일/디렉토리 없음 (완전 빈 디렉토리 + BMAD 산출물만)
- 또는 `.git/` 있지만 initial commit 없음 + 같은 조건

그 외 (이미 코드 파일 존재)면 WARN:
```
if [ -n "$(ls -A | grep -vE '^(\.git|_bmad-output)$')" ]; then
  echo "WARN: _bmad-output/ 외의 파일이 이미 존재합니다."
...
## 7단계: BMAD 스킬 설치 (산출물 아닌 스킬)

기획은 이미 완료됐지만 Loop A(구현)에서 `bmad-create-story`,
`bmad-dev-story` 등 스킬을 호출하려면 이 프로젝트 안에도 스킬이
필요합니다.
...
- 스택: <Q1 확정 결과>
- 원격: <연결됨: <URL> / 나중에>
- 커밋: 3개 (initial scaffold / harness install / BMAD skills)
...
3. BMAD skill 존재 확인:
   for d in .agents/skills/bmad-create-story \
            .agents/skills/bmad-dev-story \
            .claude/skills/bmad-code-review; do
     [ -d "$d" ] || { echo "STOP: Setup 1 BMAD 스킬 설치 단계 먼저 완료"; exit 1; }
   done
4. git hooks 활성 확인:
   [ "$(git config --get core.hooksPath)" = ".githooks" ] \
...
5. BMAD skill 경로 확인:
   for d in .agents/skills/bmad-create-story \
     ...
     [ -d "$d" ] || { echo "STOP: $d 없음. Setup 1 BMAD 스킬 설치 단계 재실행"; exit 1; }
   done
