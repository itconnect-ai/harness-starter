...
3. dev-story 완료 후 ./scripts/validate-quick.sh 실행하여 하네스 검증 (lint + typecheck + 관련 테스트)
...
모든 story 완료 후:
7. ./scripts/validate.sh 전체 실행 (Epic 단위 통합 검증)
8. 실패 시 로그 확인 후 수정: state/validate/latest/*.log
9. 수정 후 ./scripts/validate.sh --from=실패단계 로 재개 (처음부터 다시 돌리지 않음)
...
- validate-quick.sh 통과한 story만 commit + push
...
>
> **검증 흐름**: Story 단위에서는 `validate-quick.sh`(lint + typecheck + 관련 테스트)로 빠르게 검증합니다.
> Epic의 모든 Story 완료 후 `validate.sh`로 전체 통합 검증(순차 테스트 + 빌드 + 보안 + 성능)을 실행합니다.
...
> 전체 출력이 필요하면 `VALIDATE_OUTPUT_MODE=verbose`를 설정하세요.
...
3. 각 incident에 대해 재현 테스트를 tests/regression/에 작성
   ...
   다음 Epic의 validate.sh → npm run test 에서 자동으로 실행됨
4. state/learning-loop.json 업데이트 (패턴별 발생 횟수)
5. 승격 정책에 따라 조치:
   - 1회: 기록만
   - 2회: docs/agents/feedback-rules.md에 활성 규칙 추가
   - 3회+ (기계적으로 판별 가능한 경우만): scripts/validate.sh에 blocking check로 추가 (warning이 아닌 exit 1)
...
7. harness 파일을 수정했으면 반드시 검증: bash -n scripts/validate.sh && ./scripts/validate.sh
...
- 구현 완료 후 ./scripts/validate-quick.sh 실행하여 빠른 검증
- 검증 통과 후 commit + push
```
...
- AGENTS.md 규칙 준수
- docs/agents/ 규칙 참조
- 구현 완료 후 validate-quick.sh 통과 후 commit + push
```
...
│   ├── validate-quick.sh              Story 단위 빠른 검증 (lint+type+관련 테스트)
│   ├── validate.sh                    Epic 단위 전체 검증 (--from 재개 지원)
│   ├── smoke.sh                       스모크 테스트
...
3. 수정 후 `./scripts/validate.sh --from=실패단계`로 재개
...
| Harness | 호출 방법 | 대상 | 설명 |
|---|---|---|---|
| `validate-quick.sh` | `./scripts/validate-quick.sh` 또는 프롬프트에서 지시 | Phase A (Story) | 빠른 검증 (lint + typecheck + 관련 테스트) |
| `validate.sh` | `./scripts/validate.sh` 또는 프롬프트에서 지시 | Phase A (Epic) / Phase B | 전체 검증 (8단계, --from 재개 지원) |
...
완료 전 반드시:
- ./scripts/validate-quick.sh 실행하여 빠른 검증 통과
