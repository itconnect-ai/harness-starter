...
7. sprint-status.yaml은 스킬 워크플로우가 자동 업데이트
...
6. harness 파일을 수정했으면 반드시 검증: bash -n scripts/validate.sh && ./scripts/validate.sh
7. 검증 통과 후 커밋: chore(harness): Epic 1 회고 반영
...
- ./scripts/validate.sh 실행하여 검증 통과 후 commit + push
```
