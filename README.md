...
**사용자가 직접 할 일**:
1. 빈 디렉토리 만들기 (`mkdir my-project && cd my-project`)
2. 그 안에서 Claude Code 실행
3. 아래 **Phase 0 → 1 → 2** 프롬프트를 순서대로 붙여넣기 (셋업 완료)
4. 각 Epic마다 **Phase 3(Codex) → 4(Claude) → 5(Claude)** 프롬프트 반복

중간에 사용자가 대답해야 하는 것은 각 프롬프트 안의 **질문 항목 몇 개**뿐:
- Phase 0: 기술 스택, 패키지 매니저, 원격 URL (있으면)
- Phase 2: 배포 활성화 여부, DEPLOY_* secrets (있으면)

---

### Phase 0: Bootstrap (Claude Code)

Claude Code를 열고 **아래 프롬프트 전체를 복사해서 실행**하세요.
scaffold → git init → harness install → git hooks → BMAD 설치까지
한 번에 처리됩니다.

````markdown
# Harness Greenfield — Phase 0: Bootstrap

이 세션은 **빈 프로젝트 디렉토리**에서 시작합니다. 아래 순서로 실행하되,
각 단계 실패 시 STOP 조건에 따라 어디서 왜 실패했는지 명확히 출력하고
중단하세요.

## 0단계: 사용자에게 질문

아래 3개를 사용자에게 물어보고 대답을 받으세요. 대답 전에는 어떤 명령도
실행하지 마세요.
...
Q1. 기술 스택을 선택하세요:
    ...
    (3) Python (FastAPI + poetry)
    (4) Python (FastAPI + uv)
    (5) Go
    (6) Rust
    (7) Java (Maven)
    (8) Java (Gradle)
    (9) 기타 — 직접 scaffold 명령을 지정할게요

Q2. (Node.js일 때만) 패키지 매니저: npm / pnpm / yarn
    (다른 스택이면 "해당 없음")

Q3. GitHub 원격 저장소 URL (지금 연결하려면 입력, 나중에 Phase 2에서
    연결하려면 "나중에"):
    예: git@github.com:owner/repo.git
```

## 1단계: 사전 환경 체크 (실패 시 STOP)

1. 현재 디렉토리가 빈 상태인지 또는 initial commit 없는지:
   ```
   if [ "$(git log -1 2>/dev/null | wc -l)" != "0" ] || \
      [ "$(ls -A | grep -v '^\.git$' | wc -l | tr -d ' ')" != "0" ]; then
     # 빈 디렉토리 + initial commit 아님 → 주의. 이미 파일 있으면 scaffold 실패 가능.
     # README-brownfield.md 사용을 안내할지 사용자에게 확인.
     echo "WARN: 이 디렉토리에 이미 파일/커밋이 있습니다."
     echo "      기존 프로젝트라면 README-brownfield.md를 사용하세요."
     echo "      새 프로젝트 맞으면 계속 진행할 수 있습니다."
     # 사용자 명시 확인 후 진행
   fi
   ```
...
2. 필수 도구 설치 확인:
   ...
   # Node 스택이면 node + npm(/pnpm/yarn) 확인
   # Python 스택이면 python3 + 선택한 매니저 확인
   # Go/Rust/Java 스택도 동일
   ```

## 2단계: 기술 스택별 scaffold 실행

Q1 답변에 따라 아래 명령 중 하나를 실행 (사용자 확인 후):

| 선택 | 명령 |
|---|---|
| (1) Next.js | `npx create-next-app@latest . --ts --app --use-<매니저>` |
| (2) Vite+React | `npm create vite@latest . -- --template react-ts` (이후 감지된 매니저로 install) |
| (3) FastAPI+poetry | `poetry init --no-interaction` + `app/main.py` 기본 스켈레톤 생성 + `poetry add fastapi uvicorn` + `poetry add --group dev pytest ruff mypy` |
| (4) FastAPI+uv | `uv init .` + `app/main.py` 스켈레톤 + `uv add fastapi uvicorn` + `uv add --dev pytest ruff mypy` |
| (5) Go | `go mod init <사용자 질문: module path>` + `main.go` hello world + `go mod tidy` |
| (6) Rust | `cargo init .` |
| (7) Java Maven | `mvn archetype:generate -DgroupId=... -DartifactId=... -DinteractiveMode=false` |
| (8) Java Gradle | `gradle init --type java-application --dsl kotlin --test-framework junit-jupiter --project-name <이름>` |
| (9) 기타 | 사용자에게 scaffold 명령을 직접 받아 실행 |

scaffold 실패 시 STOP + 실패 로그 보고.
...
실패(curl 에러 등) 시 STOP + 네트워크 확인 안내.
...
원격 없거나 gh 인증 안 됐으면 setup-repo.sh는 자동 skip — 정상. Phase 2에서 재실행.

## 7단계: BMAD 설치 + skill 경로 검증
...
for d in .agents/skills/bmad-create-story \
  ...
  [ -d "$d" ] || { echo "STOP: $d 없음 — npx bmad-method install 재실행"; exit 1; }
done
...
- 스택: <Q1 답변 기반, 예: Next.js/TypeScript/npm>
- 원격: <연결됨: <URL> / 안 됨: Phase 2에서 연결>
- 커밋: 3개 (initial scaffold / harness install / BMAD install)

### 다음 단계

Phase 1: BMAD 기획 스킬로 PRD/Architecture/Epics 생성.
         Claude Code에서 `@.claude/skills/bmad-create-prd`를 호출하거나
         "BMAD로 PRD 만들어줘"라고 지시.

Phase 1 완료 후 Phase 2 프롬프트를 이 세션에 붙여넣기.
```

## 강제 규칙

- 사용자 Q1~Q3 답변 전에는 어떤 파일도 건드리지 말 것.
- 각 단계 실패 시 즉시 STOP + 어느 단계 어느 명령에서 실패했는지 출력.
- Phase 2로 자동 진행하지 말 것. 사용자가 Phase 1(BMAD 기획)을 완료한
  후 Phase 2 프롬프트를 직접 붙여넣어야 진행.
````

---

### Phase 1: BMAD 기획 (Claude Code)

Phase 0이 끝나면 같은 Claude Code 세션 또는 새 세션에서 **BMAD 기획
스킬**을 사용합니다. 이 단계는 대화형이라 별도 프롬프트가 필요 없습니다.

```
BMAD로 PRD, architecture, epics를 순서대로 만들어줘
```
...
Phase 1이 끝나면 Phase 2 프롬프트를 붙여넣습니다.

---

### Phase 2: Customization + Finalization (Claude Code)

Phase 1(BMAD 기획)이 끝난 뒤 **같은 Claude Code 세션에 아래 프롬프트
전체를 붙여넣기**. harness 파일 스택별 커스터마이징 + 원격 연결 +
develop 브랜치 생성 + setup-repo + (조건부) deploy 활성화까지 한 번에
처리됩니다.

````markdown
# Harness Greenfield — Phase 2: Customization + Finalization

Phase 0(Bootstrap), Phase 1(BMAD 기획)이 완료된 상태에서 시작합니다.
BMAD 산출물을 읽어 harness 파일을 실제 스택에 맞게 갱신하고, GitHub
원격·보안·배포 설정까지 마무리합니다.
...
1. harness 설치 결과물 확인:
   test -d docs/agents && test -f scripts/validate.sh \
     ...
     || { echo "STOP: Phase 0 Bootstrap 먼저 완료 (harness install + git hooks)"; exit 1; }
2. BMAD skill 존재 확인:
   for d in .agents/skills/bmad-create-story \
            .agents/skills/bmad-dev-story \
            .claude/skills/bmad-code-review; do
     [ -d "$d" ] || { echo "STOP: Phase 0 BMAD 설치 단계 먼저 완료"; exit 1; }
   done
3. git hooks 활성 확인:
   [ "$(git config --get core.hooksPath)" = ".githooks" ] \
     || { echo "STOP: Phase 0 git hooks 활성화 먼저 완료"; exit 1; }
...
사용자에게 물어보세요 (Phase 0에서 이미 원격 URL을 받았으면 skip):
...
```
# 1. origin 확인. Phase 0 Q3에서 이미 붙였으면 skip.
...
```
## Phase 2 Customization + Finalization 완료
...
Phase 3: Epic 1 구현 (Codex Desktop에서 Phase 3 프롬프트 실행)
Phase 4: Epic 1 리뷰 (Claude Code에서 Phase 4 프롬프트 실행)
Phase 5: Epic 1 회고 (Claude Code에서 Phase 5 프롬프트 실행)
```
...
아래 체크 후 하나라도 실패하면 "README.md의 Phase 2를 먼저 완료하세요"로
중단:
...
2. develop 브랜치 존재:
   git rev-parse --verify develop >/dev/null 2>&1 \
     || { echo "STOP: develop 브랜치 없음. Phase 2 원격 연결 단계 완료 필요"; exit 1; }

3. origin 존재:
   git remote get-url origin >/dev/null 2>&1 \
     || { echo "STOP: origin 미설정. Phase 2 원격 연결 단계 완료 필요"; exit 1; }
...
5. BMAD skill 경로 확인:
   for d in .agents/skills/bmad-create-story \
     ...
     [ -d "$d" ] || { echo "STOP: $d 없음. Phase 0 BMAD 설치 단계 재실행"; exit 1; }
   done
...
### Phase 5: 회고 + Harness 강화 (Claude Code)
